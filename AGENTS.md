...
- Set `generated.at` to the date the concept's content last meaningfully changed. Never omit it on a concept that lists `sources`: v0.2 makes the field optional, so a concept without it validates green and simply claims nothing.
...
- Judge freshness by comparing the newest `verified[].at` against `generated.at` yourself. An OKF consumer reads any `human:<id>` entry as human-reviewed and flags nothing when the reading predates the content, so a `verified` older than `generated.at` is honest but not current, and several concepts here are in exactly that state.
