...
- **NEVER `kubectl apply` ArgoCD-managed resources** — commit to git, push, let consumer ArgoCD reconcile. Two exceptions, both narrow: one-time bootstrap content under `kubernetes/bootstrap/`; and a **datapath break-glass** — a consumer-side operator restoring a broken CNI on a running cluster when ArgoCD itself can no longer reconcile, because it runs over the datapath Cilium provides. That one is bounded to restoring service, requires disabling auto-sync/self-heal first where the consumer added one, and requires landing the same fix in git and syncing afterwards; the hand-applied state is never the end state. It authorizes the consumer's operator, never this repository's automation, and never a module that applies cluster-side. Procedure: [`UPGRADING.md`](UPGRADING.md) §Break-glass. Neither ArgoCD's own disaster-recovery documentation nor the OpenGitOps principles define a break-glass path, so this exception is named here rather than left to a secondary document — and if it is ever reached twice for the same cause, the fix belongs upstream of it.
