...
the artifact, signature, attestations and `:latest` were all already in place.
That lookup now retries before giving up (#276), so this section applies to a
run that fails for some other reason at the same point.
