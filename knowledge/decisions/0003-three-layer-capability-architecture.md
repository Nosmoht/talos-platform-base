...
- **D7.** Workload-runtime-class labels (`platform.io/gvisor`,
...
| **C — Hardware Features Registry** | What atomic hardware predicates a node must satisfy. Referenced by Layer A entries (via `requires_hardware_features[]`) and by consumer `cluster.yaml` composite capabilities (via `requires_features[]`). | `platform-hardware-features.yaml` | `nvidia-gpu` |
...
free to vary across consumer cluster repos (a consumer may define
...
`platform-hardware-features.yaml` document the available sources;
...
Workload-runtime-class labels — `platform.io/gvisor`, future
...
(e.g., `gvisor-runtime` for `platform.io/gvisor`); ownership
...
- **Backstage entity-model adapter or Crossplane XRD mapping.** Both
  are forward-looking per [`vision.md`](../project/vision.md) and unchanged by
  this ADR.
...
> [2026-07-11 verification] The `make validate-schematics` check cited in C+2 no
> longer exists — the `make` path was retired with the OpenTofu cutover
> ([`0006-opentofu-cluster-lifecycle.md`](./0006-opentofu-cluster-lifecycle.md))
> and the Makefile at v3.0.0
> ([`0012-makefile-retirement.md`](./0012-makefile-retirement.md)). The surviving
> mechanical checks are `scripts/lint-hardware-features.sh` +
> `scripts/check-provisioning-catalog-refs.sh` (CI job `hardware-features-check`
> in `.github/workflows/gitops-validate.yml`), and consumer `cluster.yaml` is
> schema-validated by `scripts/lint-cluster-yaml.sh` against
