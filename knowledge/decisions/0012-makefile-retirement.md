...
deciders:
  - platform-maintainer
supersedes:
  - /decisions/0008-task-runner-consolidation.md
related:
...
ADR executes its §4 convergence.

> [2026-07-11 verification] The tracked `kubernetes/base/infrastructure/` set
> has since narrowed to `{argocd}` only: the idle cert-approver stub was removed
> at v3.0.0 when cert-approver became a Talos controlplane `inlineManifest` seed
> ([0013-kubelet-serving-cert-rotation.md](./0013-kubelet-serving-cert-rotation.md)).
> CI pins the renderable component set via `.ci-renderable-components.txt`
> (content: `argocd`), enforced by the `cmp` gate in
> `.github/workflows/gitops-validate.yml`. It is filed as a **supersession** rather than a
...
> [2026-07-11 verification] The one-release deprecation stub is still present at
> the repo-root `Makefile` as of v3.x (latest tag v3.0.0): any `make <target>`
> prints the migration mapping and exits 2 (observed in-session). Its removal
> trigger remains the next MAJOR release, which has not yet occurred.
