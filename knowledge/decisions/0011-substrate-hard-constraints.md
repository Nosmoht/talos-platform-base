...
(the platform policy-stack decision record), "never `kubectl apply` ArgoCD-managed" by the
...
> [2026-07-11 verification] Enforcement is live and CI-gated exactly as recorded:
> `.github/workflows/hard-constraints-check.yml` carries all three steps — the
> SecureBoot step greps `(metal-secureboot|installer-secureboot)` over changed
> `tofu/**` files, the kernel-parameter step greps `debugfs=off` over
> `kubernetes/**` + `tofu/**`, and the forbidden-kinds step greps
> `^kind:[[:space:]]*(Ingress|Endpoints)\b` over `kubernetes/**` (all three skip
> `*.md`, scanning PR-changed files only). In module code,
> `tofu/modules/talos-cluster/main.tf` selects `urls.installer` (never
> `urls.installer_secureboot`) and carries the `secureboot_patches` plan-time
> precondition. The frontmatter `status: proposed` predates this enforcement —
> the record is retroactive over already-enforced invariants; flipping the
> status is a maintainer decision, not part of this verification.
