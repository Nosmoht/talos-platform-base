...
- the `make` / `talos/scripts/argv-print.sh` path, which emits
  `talosctl gen config` argv for bare-metal nodes (the bare-metal
  kitchen-sink archetype);
- a `tofu/modules/talos-cluster` HCL path (Crossplane-provisioned) that
...
[`0004-substrate-only-base.md` §Validation](./0004-substrate-only-base.md#validation)
...
Crossplane (heterogeneous hardware × tofu provisioning) would
therefore have to duplicate the bare-metal path's accumulated patch
...
- Con: divergence is structural, not incidental. The
  ...
  never consumes — its only consumer is an external provisioning frontend's own Taskfile)
...
- `0004-substrate-only-base.md` — the substrate model whose
  realisability this four-cluster investigation validated.
- `0001-multi-repo-platform-split.md` — OCI consumption and
