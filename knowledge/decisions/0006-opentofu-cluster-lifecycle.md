...
deciders:
  - platform-maintainer
supersedes:
  - /decisions/0005-shared-render-artifact.md
related:
...
> [base:node-capability-composition](./0009-node-capability-composition.md): a node
...
- The base must stay substrate-only and identity-free
  ([0004-substrate-only-base.md](./0004-substrate-only-base.md)); the module is
...
- Negative: breaking change for consumers — every consumer migrates its Talos
  ...
  **[Amended by [base:cluster-yaml-sot](./0007-cluster-yaml-sot.md): `cluster.yaml`
  ...
  `platform-hardware-features.yaml` catalogue stays — it is PNI
...
- `tofu/modules/talos-cluster/examples/complete/` validates a mixed
...
- This ADR is wrong if a consumer cannot express its cluster against the module
  ...
  migration (the complete example) is the test; the PKI-adoption follow-up is the known gap.
...
> [2026-07-11 verification] Realized — the asymmetry is closed: `deploy_cilium`
> (default `true` in `tofu/modules/talos-cluster/variables.tf`) renders the
> Cilium chart via `data.helm_template` into the controlplane `inlineManifests`
> and sets `cluster.network.cni.name: none`, per
> [`0007-cluster-yaml-sot.md`](./0007-cluster-yaml-sot.md). Delivery via consumer
> `config_patches` is no longer the default path (it remains only as the
> `deploy_cilium = false` opt-out).
...
- [0004-substrate-only-base.md](./0004-substrate-only-base.md) — the substrate-only boundary this module respects.
