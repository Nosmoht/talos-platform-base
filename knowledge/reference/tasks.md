...
| `tofu:check:provider-document-kinds` | CI fence: which Talos machine-config document KINDS the pinned `siderolabs/talos` provider accepts in `config_patches`, and that a Talos 1.14 pin still yields none of the 1.14 default documents. The provider decodes patches against its own bundled Talos machinery, so the module's four opaque patch lists reach the PROVIDER's document surface rather than Talos' — a coupling no other gate observes. Two of its four cases assert a REJECTION, so the fence expires by design: it turns red once a provider inside the declared range accepts the 1.14 kinds. Probes a locally generated PKI, so no cluster and no Image Factory; requires `jq`. Runs `scripts/check-provider-document-kinds.sh`. |
