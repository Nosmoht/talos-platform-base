...
| `tofu:ci` | Aggregate: `fmt:check` + in-repo `validate` + extracted OCI module validation + `lint` + `check:render-determinism` + `check:readme-parity` + `check:kubeconfig-endpoint-regen` + `check:node-projection-wiring` + `check:shim-key-parity` + `test:offline` — mirrors CI, stays offline except for provider installation during `tofu init`. |
