...
| `tofu:check:provider-document-kinds` | CI fence: which Talos machine-config document KINDS the pinned `siderolabs/talos` provider accepts in `config_patches`, what a Talos version pin makes it generate, and that all five sites carrying the provider pin agree. The provider decodes patches against its own bundled Talos machinery, so the module's four opaque patch lists reach the PROVIDER's document surface rather than Talos' — a coupling no other gate observes. Since the pin to `0.12.0-beta.0` it asserts acceptance by rendered VALUE (the 1.14 kinds are generated by default, so presence would prove nothing), keeps one invented kind that must still be refused, checks the 1.14 defaults by value at 1.14 and their absence at 1.13.9, and covers the generated `UnattendedInstallConfig` that ignores `machine.install` plus the patch that remedies it. The last two are expiry alarms: they turn red when the provider improves, which is the signal to revisit the 1.13.9 example pins. Probes a locally generated PKI, so no cluster and no Image Factory; requires `jq`. Runs `scripts/check-provider-document-kinds.sh`. |
| `tofu:check:provider-document-kinds-bites` | Mutation bite-check for the fence above: mutates a copy of the script per expectation and requires that assertion's own error message, plus a control run of the unmutated fence whose output and exit class it propagates. Exists because one exit code covers many assertions over provider-rendered text, so a stale assertion would report OK forever. Binds every assertion an expectation-level mutation can falsify; the provider-behaviour branches are deliberately unbound. Runs ahead of the fence in `tofu:ci`. Runs `scripts/check-provider-document-kinds-bites.sh`. |
...
| `tofu:ci` | Aggregate: `fmt:check` + in-repo `validate` + extracted OCI module validation + `lint` + `check:render-determinism` + `check:readme-parity` + `check:kubeconfig-endpoint-regen` + `check:node-projection-wiring` + `check:shim-key-parity` + `check:provider-document-kinds` + `test:offline` — mirrors CI, stays offline except for provider installation during `tofu init`. |
...
| `supply-chain:check-release-step` | Proves `oci-publish.yml`'s two release steps bite, via `scripts/check-release-step-bites.sh`. The steps only ever run on a tag push and a published release is immutable, so a defect there costs a version rather than a re-run; the bite-check EXTRACTS them from the workflow (a copy would keep passing after the workflow changed) and drives them against a stub `gh` through every release state: absent, leftover draft, two drafts, already published, and a draft whose asset upload died part-way. It asserts the ORDER of the `gh` calls, because a release created and a release whose assets arrived before it was published have the same exit status. Three further assertions cover what the steps depend on rather than what they do: that `.releaserc.json` declares no publish plugin and that nothing shadows that file (the fix's other half is an ABSENCE, and `@semantic-release/github` ships as a dependency of semantic-release and sits in its default plugin list), and that no `run:` block anywhere in the workflow splices a `${{ }}` expression into the shell. The target asserts the completion marker, not just the exit code. Run verbatim by `docs-lint.yml`. |
