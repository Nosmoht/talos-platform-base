...
**Chart-pin parity** covers both version and archive digest between the seed
(`variables.tf` plus `chart-integrity.tf`) and steady state (`chart.lock.yaml`) —
invariant P above. A same-version upstream republish is rejected before the
module renders it.
