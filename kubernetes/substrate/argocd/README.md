...
**Reading note (I6) — five policies, six workloads.** `argocd-applicationset-controller` is deliberately **not** policed, and that is upstream's design rather than a gap the base should close. Its template is gated on `applicationSet.metrics.enabled or ingress.enabled or httproute.enabled` in addition to `global.networkPolicy.create`, and the base enables none of the three — so an emitted policy would carry an empty ingress list, which for an `Ingress`-type policy is a **default deny** on the pod that receives SCM webhooks (`webhook` :7000) and serves the applicationset API to repo-server. The consequence to be explicit about: that pod is unselected by any policy, therefore unrestricted, so its `webhook`, `metrics` and `probe` ports are reachable from every pod in every namespace. A consumer who enables `applicationSet.metrics` in their own values legitimately makes the set six, and I6's anchor will fail until the sixth policy is added to the contract — which is the intended prompt, not a false positive.
