...
| **I6** | Both renders carry the exact five-policy NetworkPolicy posture: each policy selects its named Argo CD component, redis and repo-server admit only their documented callers and ports, metrics stay reachable, and `argocd-server` admits every source. | `argo-cd` 10.0.0 flipped `global.networkPolicy.create` to `true` and the base carries no override, so the policies are the substrate's shipped network posture — Cilium enforces them in every consuming cluster. [`check-argocd-network-policy-invariants.sh`](../../../scripts/check-argocd-network-policy-invariants.sh) binds the five-policy anchor plus each policy's `apiVersion`, `namespace` and complete `spec` (selector, ingress peers and ports, `policyTypes`) in both render paths, asserts no higher-precedence policy object (`AdminNetworkPolicy`, Cilium policy CRDs) rides along where the `kind` filter could not see it, and cross-checks every **named** ingress port against the target workload's `containerPort` names — a rename there turns an allow rule into a rule matching nothing while every policy document stays byte-identical. For the steady state it runs on the **kustomize-built** component as well as on the fresh helm render: the committed `_rendered/` tree is a file consumers vendor, so a hand edit to it — or a regeneration through a changed `_rendered-overlay/` — reaches them while a fresh-render check stays green, and `verify-rendered.sh` proves only that `_rendered/` is *reproducible*, never that what it reproduces satisfies I6. A set that no longer matches is a **violation** (exit 3), not a render-shape error: unlike I1-I5 this invariant is positive, so the posture disappearing IS the failure. Pinning the open `argocd-server` rule is the one clause asserting the *least* restrictive shape in the set, and that is deliberate — it is the only guard against a chart or values change silently making Argo CD unreachable in every consuming cluster, so adopting a narrower upstream policy is a conscious edit here plus a consumer-facing note. [`check-argocd-network-policy-gate-bites.sh`](../../../scripts/check-argocd-network-policy-gate-bites.sh) mutates the committed render to prove the gate rejects an empty selector, a selector targeting an unrelated component, open redis ingress, closed server ingress, a wrong port, a wrong `policyTypes`, a vanished policy, an added sixth, a namespace move, a foreign policy kind and a renamed `containerPort`. |
...
**Reading note (I6) — five policies, six workloads.** `argocd-applicationset-controller` is **not** policed. The chart's template is gated on `applicationSet.metrics.enabled or ingress.enabled or httproute.enabled` in addition to `global.networkPolicy.create`, and the base enables none of the three, so no policy is emitted for it. The consequence to be explicit about: that pod is selected by no policy, therefore unrestricted, so its `webhook` (:7000), `metrics` (:8080) and `probe` (:8081) ports are reachable from every pod in every namespace. Enabling `applicationSet.metrics` alone is **not** the fix: inside the template the metrics rule is unconditional while the webhook rule is gated on `ingress`/`httproute`, so metrics-only emits an `Ingress`-type policy permitting metrics and nothing else — which default-denies the webhook port on the pod that receives SCM webhooks. A consumer who does enable one of the three legitimately makes the set six, and I6's anchor will fail until the sixth policy is added to this contract; that is the intended prompt, not a false positive.
