...
1. **Combinatorial class explosion.** A node's hardware function is a *set* of
   ...
   homelab `gpu` class already re-lists every base extension because YAML
   anchors cannot append
   (`tofu/modules/talos-cluster/examples/homelab/cluster.yaml`).
...
2. **Kernel arguments are in the wrong layer and are a no-op.** Boot-time
   ...
   homelab `kubevirt` class (`intel_iommu=on` via `machine.install`) is a
...
- **Follow-up.**
  ...
  4. Migrate the homelab example (its `kubevirt` IOMMU is the live bug) + a
