...
- [`adr-shared-render-artifact.md`](adr-shared-render-artifact.md) — why a single shared render artifact (`argv-print.sh` `EMIT=content`) is the cross-frontend source of truth for per-node Talos config, instead of two divergent renderers. **Status: accepted.**
