...
in the harness plugin's session-optimization rule.)
...
The `/implement-issue` Skill (provided by the harness plugin)
...
**This phase mapping is the contract for the harness-plugin Skill edit (Phase B
...
| `/implement-issue` skill | ✅ (harness plugin) | ❌ (no equivalent skill) |
