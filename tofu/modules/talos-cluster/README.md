...
| `cilium_k8s_service_host` | string | `"localhost"` | `k8sServiceHost` — the endpoint Cilium reaches the API server through before the CNI is up. Default is Talos KubePrism. Set a VIP or load-balancer address on a cluster where KubePrism is not the intended path. Only emitted when `cilium_kube_proxy_replacement=true`. Must be a bare DNS name/IPv4 literal, or an **unbracketed** IPv6 literal (`2001:db8::1`) — no scheme, no `:port` (that is its own input), no brackets. Brackets are rejected on purpose: the chart passes this to `KUBERNETES_SERVICE_HOST`, and client-go joins host and port with `net.JoinHostPort`, which brackets a colon-bearing host again, so `[2001:db8::1]` would reach the API server as `[[2001:db8::1]]:6443`. Install-time-fixed on the seed; measured sink: chart 1.20.0 renders it into the containers' `KUBERNETES_SERVICE_HOST` env var, and on the seed path that render is frozen into a create-only machine config, so a malformed host is a bootstrap deadlock. |
