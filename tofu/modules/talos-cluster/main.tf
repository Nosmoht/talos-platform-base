...
locals {
  ...
  # upgrade in place without drain. Gated on auto_os_upgrade so the ephemeral
  # kubeconfig below only materialises when tofu drives upgrades.
...
# per-node install.image + HostnameConfig + capability/node patches are folded
# into one config_patches list. Strategic-merge order is preserved from the
# two-pass era — base_cluster FIRST (caller-overridable), then caller all-node +
# role patches, then the module install.image, then the module-generated
# capability patch then node patches (so a node patch can still override
# install.image, as before), then the controlplane-only inlineManifest seeds, and
# base_cni_patch LAST so the authoritative cni:none + proxy always win over any
# node patch.
data "talos_machine_configuration" "per_node" {
  ...
  config_patches = concat(
    ...
    # base_cni_patch LAST so cni:none + proxy win over any node patch (when
...
resource "talos_machine" "this" {
  ...
  #     above; OS upgrades stay the consumer's controlled out-of-band upgrade
  #     path — unchanged from the apply era.
  #   true → image = the per-node non-secureboot installer → bumping the node's
  #     install version drives an in-place OS upgrade on the next apply (no
  #     talosctl, no manual reboot).
  image = var.auto_os_upgrade ? data.talos_image_factory_urls.this[local.node_install_key[each.value.hostname]].urls.installer : null
  ...
  # sequencing. Single-node clusters are unaffected.
}
